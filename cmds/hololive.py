...
@commands.command(
    name="hololive",
    aliases=["schedule", "holoschedule"],
    help="Gets interesting streams from hololive",
)
async def run(ctx: commands.context.Context):
    ...
    pages = []
    offset_minutes = int(
        get_localzone().utcoffset(datetime.datetime.utcnow()).total_seconds() / 60
    ...
    for day, day_streams in days.items():
        ...
        for stream in day_streams:
            ...
            entries.append(
                "{}({})".format(
                    stream_starttime.humanize(),
                    (
                        stream.starttime + datetime.timedelta(minutes=offset_minutes)
                    ).strftime("%I:%M %p"),
                )
            )
            entries.append("")
